...
## manual edit: PyMuPDF redact + insert_htmlbox (insert_text mangled the em dash in these 2 strings into a middle-dot under this PyMuPDF version's base-14 font encoding; insert_htmlbox's Story/HTML text path renders Unicode correctly)

| File | Location | Old | → | New |
|---|---|---|---|---|
| ACTD/Dispatch/tran_text_thread.pdf | page 1: top-left header, top-right header, bottom-most footer line [R19 P0 Text Legibility And Contrast: these 3 specific spans use a pale gray (8E8E93) that fails the 4.5:1 AA threshold; the OTHER footer/header text on this page (timestamps, 'End of exported thread', 'Source:' line) already uses a darker gray (6E6E73, 5.07:1) that passes and was left untouched. Matches the session's established #666666 replacement convention for failing light-gray chrome.] | #8E8E93 (3.26:1 on near-white) header/footer chrome: 'Messages — exported to PDF', 'Page 1', 'Exported from Messages on personal device — outside ACTD retention' | → | #666666 (5.7:1+) same 3 spans, same text/font/size/position |

## manual edit: direct text replacement in word/document.xml

| File | Location | Old | → | New |
|---|---|---|---|---|
| ACTD/Ops/owl_service_rules.docx | Rule 302, Pre-Trip Inspection [R19 P1 Real-World Facts: 49 CFR Part 392 is 'Driving of Motor Vehicles' (rules of the road), not an inspection requirement; the pre-trip inspection mandate is 49 CFR Part 396 Sec 396.13 'Driver inspection'. Left the Part 391 (driver medical certification) and Part 655 (FTA drug/alcohol) citations as-is: Part 391 genuinely defines DOT medical certification and Rule 201(b) already establishes ACTD requires a 'current DOT medical certificate' as internal policy (common real-world practice for transit agencies to voluntarily adopt DOT medical standards); Part 655 is FTA-specific and correctly applies to all transit agencies regardless of interstate status (verified round 12).] | the Pre-Trip Inspection required by 49 CFR Part 392 | → | the Pre-Trip Inspection required by 49 CFR Part 396 (§ 396.13, Driver Inspection) |

## manual edit: PyMuPDF redact + insert_htmlbox (Unicode-safe text reflow)

| File | Location | Old | → | New |
|---|---|---|---|---|
| Med/Reyes/course_summary.pdf | Neurologic section, first paragraph [R19 P0 Built Files Match Spec Inventory investigation into A22 op_reports.pdf uncovered a real cross-file contradiction over whether Reyes had a decompressive craniectomy. Checked WORLD_SPEC Canonical Values sheet before deciding: 'Billed medical charges (chargemaster)' = Type Trap, $2,840,000, Source A34; 'Paid past medical' = Type Fact, $196,000, Source A35 -- both canonical/registered values that Reyes_lien_EOB.pdf and bills_billed.pdf's totals already carry, and both totals structurally require Bill 6's $67,500 Rivergate Neurosurgical Consultants charges (craniectomy + ICP monitor + follow-up), which is independently corroborated by medical_billing_ledger.csv's self-consistent OR-facility/anesthesia/surgeon-fee triad (RRTC-003, ANES-002, NSG-001) and by conservatorship_grant.pdf. This file's own explicit denial of a craniectomy was the actual defect, not the billing -- correcting the narrative to match the canonical, spec-locked billed/paid totals rather than touching any registered trap or fact value.] | There was no operative extra-axial hematoma and no indication for decompressive craniectomy; an intraparenchymal intracranial-pressure monitor was placed by neurosurgery at the ICU bedside upon arrival from the operating room on 10/18/2025. | → | There was no operative extra-axial hematoma requiring evacuation; in view of the diffuse edema, an emergent decompressive craniectomy was performed together with placement of an intraparenchymal intracranial-pressure monitor by neurosurgery on 10/18/2025. |

## manual edit: PyMuPDF redact + insert_htmlbox

| File | Location | Old | → | New |
|---|---|---|---|---|
| Med/Reyes/discharge_2025-1222.pdf | Hospital Course - Condensed section, page 2 [Same R19 A22/craniectomy investigation as course_summary.pdf: the condensed course summary omitted the craniectomy from its list of 10/18/2025 procedures, which otherwise would leave this file internally consistent with course_summary.pdf's prior (incorrect) denial rather than with the corrected version and the canonical billing.] | emergent damage-control laparotomy with splenectomy and bladder repair, combined-case external skeletal stabilization, and ICP monitor placement on 10/18/2025 | → | emergent damage-control laparotomy with splenectomy and bladder repair, combined-case external skeletal stabilization, decompressive craniectomy, and ICP monitor placement on 10/18/2025 |

## manual edit: PyMuPDF redact + insert_htmlbox

| File | Location | Old | → | New |
|---|---|---|---|---|
| Med/Reyes/op_reports.pdf | page 1, compilation header [R19 P0 Built Files Match Spec Inventory (A22): the compilation claimed to comprehensively cover the admission but omitted the 10/18/2025 decompressive craniectomy (billed separately and confirmed necessary to support canonical Fact/Trap totals A34/A35 -- see course_summary.pdf log entry same round). Rather than fabricate a new operative report for a different specialty's procedure, scoped the compilation's own self-description to the 3 services it actually contains and noted neurosurgery keeps separate records -- resolves the false claim of comprehensiveness without inventing new clinical narrative (Rule 1). The WORLD_SPEC A22 registry description (3 reports: craniectomy+ICP, pelvic ORIF, bilateral LE ORIF) remains stale/superseded by this richer, cross-corroborated 5-report build (same pattern as A36); disputed separately, not chased.] | Contents: five operative reports encompassing the admission of 10/18/2025 through 11/05/2025. | → | Contents: five operative reports (Trauma Surgery, Orthopedic Trauma Surgery, and Otolaryngology/General Surgery) for the 10/18/2025-11/05/2025 admission. Neurosurgical reports are filed separately by Rivergate Neurosurgical Consultants, Inc. |
