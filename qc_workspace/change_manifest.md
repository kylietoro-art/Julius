...
| File | Location | Old | → | New |
|---|---|---|---|---|
| ACTD/CAD/AVL_GPS_track_2025-1018.csv | odometer_mi column, all 222 data rows [wider sweep (not an AutoQC ticket): cross-checked maintenance_log_bus4177.csv against AVL telemetry for the same vehicle/night and found an unreconciled mileage jump] | 187342.6 - 187367.7428 mi (implies ~39,140 mi driven in the 7 days since the 10/10/2025 maintenance-log entry at 148,203 mi -- physically impossible) | → | 149533.0 - 149558.1428 mi (constant -37,809.6 shift; preserves every delta from the physics-based Trip-6 reconstruction exactly, rebases only the absolute baseline to ~7 days x ~190 mi/day past the last maintenance-log reading) |

## manual edit

| File | Location | Old | → | New |
|---|---|---|---|---|
| Shared/Press/news_collision.html | article body para 2; meta published-time tag [wider sweep (not an AutoQC ticket): T2's registered trap only covers the 0.11% roadside BAC and the passenger count, not the streetlight side, so the west/east contradiction was an unintended defect, not a protected trap value] | 'struck a City-owned street-light standard on the west side of Main'; <meta name="published-time" content="14:42 PT"> | → | '...on the east side of Main' (matches TC_2025-1018.pdf's NE-corner/east-curb account and onboard_video_still_log.pdf's 'east curb, south of Third Avenue'); meta published-time now 06:18 PT to match the article's own stated original publish time rather than its Updated time |

## manual edit

| File | Location | Old | → | New |
|---|---|---|---|---|
| Shared/CrimeLab/chain_custody.pdf | p.2 Collector/Phlebotomist-signature/Released-by/Released-signature fields; p.4 tube-seal initials line [wider sweep (not an AutoQC ticket): unintended name collision between two unrelated characters at the same facility on the same night, a Personnel Names/Identifiers Consistency risk; the phlebotomist's identity carries no trap content, so renaming has zero blast radius] | Phlebotomist/collector identified as 'Priya J. Ramaswamy' / 'P. Ramaswamy, CPT-II' -- same first+last name as the extensively-documented trauma attending 'Priya Ramaswamy, MD' who treats Reyes at the same hospital that same night, with nothing anywhere signaling they are meant to be the same or different people | → | Renamed the phlebotomist to 'Diane M. Castellano' / 'D. Castellano, CPT-II' throughout (5 instances, 2 pages); verified 'Castellano' appears nowhere else in the corpus |
